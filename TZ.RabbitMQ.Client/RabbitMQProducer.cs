...
namespace TZ.RabbitMQ.Client
{
    public class RabbitMQProducer : RabbitMQClientBase,IDisposable
    {
        ...
        /// <param name="queueName"></param>
        /// <param name="prefetchCount">默认20</param>
        public bool SendWorkQueuesWithConfirmSelect(string message, string queueName)
        {
            var routingKey = queueName;
            Channel.QueueDeclare(queue: queueName,
                durable: true,
            ...
            Channel.BasicPublish(exchange: "",
                ...
                basicProperties: properties,
                body: body);
            var isPublished = Channel.WaitForConfirms();//等待通道确认是否发送成功
            return isPublished;
        }
        ...
        /// <param name="queueName"></param>
        /// <param name="prefetchCount">默认20</param>
        public bool SendDelayQueuesWithConfirmSelect(string message, string queueName,double delayMilliseconds,string beDeadLetterPrefix="beDeadLetter_")
        {
            #region 死信到期后转入的交换机及队列
            //死信转入新的队列的路由键（消费者使用的路由键）
            var routingKey = queueName;
